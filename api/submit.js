...
async function ensureSchema(sql){
  await sql`CREATE TABLE IF NOT EXISTS livingston_visit_submissions (
    id UUID PRIMARY KEY,
  ...
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS visit_date DATE`;
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS visit_time TIME`;
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS arrival_time TIME`;
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS departure_time TIME`;
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS client_first_name TEXT`;
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS client_last_name TEXT`;
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS arrival_guardian_name TEXT`;
  await sql`ALTER TABLE livingston_visit_submissions ADD COLUMN IF NOT EXISTS departure_guardian_name TEXT`;

  const oldColumn=await sql`SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='livingston_visit_submissions' AND column_name='submitted_at'`;
  if(oldColumn.length){
    await sql`UPDATE livingston_visit_submissions SET
    ...
    await sql`ALTER TABLE livingston_visit_submissions DROP COLUMN submitted_at`;
  }
  await sql`ALTER TABLE livingston_visit_submissions ALTER COLUMN visit_date SET NOT NULL`;
  await sql`ALTER TABLE livingston_visit_submissions ALTER COLUMN visit_time SET NOT NULL`;
  await sql`UPDATE livingston_visit_submissions SET arrival_time=visit_time WHERE arrival_time IS NULL`;
  await sql`UPDATE livingston_visit_submissions SET
  ...
  await sql`UPDATE livingston_visit_submissions SET arrival_guardian_name=COALESCE(arrival_guardian_name,parent_name)`;
}
...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    if(visitAction==="arrival"){
      const open=await sql`SELECT id FROM livingston_visit_submissions WHERE visit_date=(CURRENT_TIMESTAMP AT TIME ZONE 'America/New_York')::date AND LOWER(TRIM(client_first_name))=LOWER(${clientFirstName}) AND LOWER(TRIM(client_last_name))=LOWER(${clientLastName}) AND departure_time IS NULL LIMIT 1`;
      ...
      const rows=await sql`INSERT INTO livingston_visit_submissions (id,parent_name,arrival_guardian_name,departure_guardian_name,client_name,client_first_name,client_last_name,visit_date,visit_time,arrival_time,departure_time)
    ...
    const rows=await sql`UPDATE livingston_visit_submissions SET departure_time=${departureTime}::time,departure_guardian_name=${parentName} WHERE id=${open[0].id}::uuid RETURNING visit_date,arrival_time,departure_time`;
